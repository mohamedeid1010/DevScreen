...
function renderWithBadges(text: string, repo: string, commitRef: string | null) {
  return text.split(/(\{repo\}|\{commitRef\})/g).map((segment, i) => {
    if (segment === "{repo}") {
      return (
        <span key={i} className="mx-0.5 inline-flex items-center gap-1 rounded-md border border-[#f99c0020] bg-[#f99c000d] px-1.5 py-0.5 font-mono text-xs text-[#f99c00]">
    ...
    if (segment === "{commitRef}" && commitRef) {
      return (
        <span key={i} className="mx-0.5 inline-flex items-center gap-1 rounded-md border border-[#ac4bff33] bg-[#ac4bff0d] px-1.5 py-0.5 font-mono text-xs text-[#ac4bff]">
        ...
        <div className="mt-4 grid gap-5 xl:grid-cols-[1.1fr_0.9fr] xl:items-start">
          <div className="space-y-3">
            ...
            <p className="max-w-xl text-sm leading-7 text-[#a8a29e]">
              Every question is anchored to a specific repository and — where applicable — a concrete code
              snippet extracted from the candidate&apos;s own commit history. Questions adapt based on detected
              skill vectors.
            </p>
          </div>

          <div className="rounded-xl border border-[#ffffff0a] bg-[#0c0c0e] p-5">
            <p className="mb-3 font-mono text-xs uppercase tracking-widest text-[#a8a29e]">Session Parameters</p>
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={item.label} className="rounded-xl border border-[#ffffff0a] bg-[#111113] p-3">
          ...
          {questions.map((q, i) => (
            ...
            >
              <div className="mb-4 flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full border border-[#ffffff0a] bg-[#ffffff07] px-2.5 py-0.5 text-xs text-[#a8a29e]">{q.type}</span>
                <span className={`rounded-full border px-2.5 py-0.5 font-mono text-xs font-medium ${q.difficultyClass}`}>{q.difficulty}</span>
                <div className="ml-auto flex items-center gap-1.5 rounded-full border border-[#f99c0020] bg-[#f99c000d] px-2.5 py-1">
                  ...
                  {q.commitRef && <span className="font-mono text-[10px] text-[#a8a29e]/50">#{q.commitRef}</span>}
                </div>
              </div>
              ...
              <div className="mt-3 rounded-xl border border-[#ffffff0a] bg-[#0c0c0e] p-4">
                <div className="mb-1.5 flex items-center gap-2">
                  ...
                  <p className="font-mono text-[10px] uppercase tracking-widest text-[#a8a29e]/50">Interviewer Signal</p>
                </div>
          ...
          <div className="sticky top-24 space-y-4">
            <section className="rounded-2xl border border-[#ffffff12] bg-[#0c0c0e] p-6">
              <div className="mb-4 flex items-center gap-2">
                ...
                <p className="font-mono text-xs font-semibold uppercase tracking-widest text-[#a8a29e]">Scoring Rubric</p>
              </div>
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={i} className="flex items-start gap-2.5 rounded-xl border border-[#ffffff0a] bg-[#111113] px-3 py-3">
                    <span className="mt-0.5 shrink-0 font-mono text-[10px] text-[#a8a29e]/40">{String(i + 1).padStart(2, "0")}</span>
            ...
            <section className="rounded-2xl border border-[#ffffff12] bg-[#111113] p-5">
              <p className="mb-3 font-mono text-xs uppercase tracking-widest text-[#a8a29e]">Referenced Repositories</p>
              <div className="space-y-2">
                {repoSources.map((repo) => (
                  <div key={repo.name} className="flex items-center justify-between rounded-xl border border-[#ffffff0a] bg-[#0c0c0e] px-3 py-2.5">
                    ...
                    <div className="flex items-center gap-2 font-mono text-[10px] text-[#a8a29e]/50">
                      ...
                      <span className="rounded-full bg-[#ffffff0a] px-1.5 py-0.5">{repo.questions}q</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="space-y-2.5 rounded-2xl border border-[#ffffff12] bg-[#111113] p-5">
              ...
              <Button asChild variant="outline" className="h-10 w-full rounded-xl border-[#ffffff12] bg-[#0c0c0e] text-sm text-[#f2eae3] hover:bg-[#17171a] hover:text-[#f2eae3]">
