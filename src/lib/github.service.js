...
export async function fetchLanguageStats(username) {
  ...
  await Promise.all(
    repos.slice(0, MAX_REPOS_TO_SCAN).map(async (repo) => {
      try {
 ...
 */
export async function fetchCommitActivity(username) {
  const octokit = getOctokit();
  const repos = await fetchUserRepos(username);
  const commits = [];
 ...
 */
export async function fetchRepoCode(username, repoName, branch = "main") {
  const octokit = getOctokit();
  const files = [];
  ...
  try {
    ...
    const candidates = tree.tree.filter((node) => {
      ...
      if (node.size > MAX_FILE_SIZE_BYTES) return false;
